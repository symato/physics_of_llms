# source https://raw.githubusercontent.com/axolotl-ai-cloud/axolotl/main/src/axolotl/monkeypatch/unsloth_.py
...
def integrate_cross_entropy_loss_patch(model_type: str = "llama") -> None:
    if model_type == "llama":
        ...
        print("INFO: patching unsloth fast_cross_entropy_loss")
...
def patch_self_attn_lora():
    ...
    print("INFO: patching unsloth attn lora")
...
def integrate_rope_embeddings():
    ...
    print("INFO: patching unsloth RoPE embeddings")
...
def integrate_lora_mlp_patch(peft_model: PeftModelForCausalLM):
    ...
    for idx, layer in enumerate(peft_model.model.model.layers):
        ...
        if is_mlp_lora and mlp_no_bias and mlp_not_dora:
            ...
            print("WARNING: unable to apply unsloth lora mlp patch to layer %d", idx)
            pass
    ...
    for idx, layer in enumerate(peft_model.model.model.layers):
        if cfg.unsloth_lora_qkv:
            ...
            if is_qkv_lora and qkv_no_bias and qkv_not_dora:
                ...
                print("WARNING: unable to apply unsloth lora qkv patch to layer %d", idx)
        if cfg.unsloth_lora_o:
            ...
            if is_o_lora and o_no_bias and o_not_dora:
                ...
                print("WARNING: unable to apply unsloth lora o_proj patch to layer %d", idx)
...
def patch_unsloth_layernorm():
    try:
        ...
        print("INFO: patching with unsloth.kernels.rms_layernorm")
        transformers.models.llama.modeling_llama.LlamaRMSNorm = LlamaRMSNorm
    except ImportError:
        print("WARNING: missing unsloth library")
